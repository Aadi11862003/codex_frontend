...
const QuickSortVisualizer = () => {
    ...
    const partition = (arr, low, high, steps) => {
        ...
        for (let j = low; j < high; j++) {
            steps.push({ array: [...arr], comparing: [j, high], line: 9 });
            if (arr[j] < pivot) {
                i++;
                [arr[i], arr[j]] = [arr[j], arr[i]];
                steps.push({ array: [...arr], swapping: [i, j], line: 11 });
            }
        }
        [arr[i + 1], arr[high]] = [arr[high], arr[i + 1]];
        steps.push({ array: [...arr], swapping: [i + 1, high], line: 14 });
        return i + 1;
    };
